...
class BasicAgent(object):
    ...
    def _obstacle_avoid_manager(self, waypoint, vehicle, distance):
        """
        Attempts to navigate around a blocking obstacle by temporarily
        using the opposite/adjacent lane.

        Called when a static prop or very slow vehicle is blocking the
        ego lane and the agent has nearly stopped.

            :param waypoint: current ego waypoint
            :param vehicle: the blocking actor
            :param distance: distance to the blocking actor (meters)
            :return: carla.VehicleControl if a bypass was planned, None otherwise
        """
        if self._speed > 5.0:
            return None
        if distance > 20.0:
        ...
        obstacle_half_len = max(
            vehicle.bounding_box.extent.x,
            vehicle.bounding_box.extent.y
        )
        bypass_distance  = distance + obstacle_half_len + 4.0
        return_distance  = bypass_distance + obstacle_half_len + 8.0 

        bypass_wps = waypoint.next(bypass_distance)
        if not bypass_wps:
            return None
        bypass_entry_wp = bypass_wps[0]

        return_wps = waypoint.next(return_distance)
        if not return_wps:
            return None
        return_entry_wp = return_wps[0]

        left_wpt  = bypass_entry_wp.get_left_lane()
        right_wpt = bypass_entry_wp.get_right_lane()

        all_vehicles = list(self._world.get_actors().filter("*vehicle*"))

        for bypass_wpt, lane_offset in [(left_wpt, -1), (right_wpt, 1)]:
            if bypass_wpt is None:
                continue
            if bypass_wpt.lane_type != carla.LaneType.Driving:
                continue
            if waypoint.lane_id * bypass_wpt.lane_id < 0:
                clear_distance = return_distance + 10.0
            else:
                clear_distance = bypass_distance + 5.0

            lane_clear, _, _ = self._vehicle_obstacle_detected(
                all_vehicles,
                max_distance=clear_distance,
                up_angle_th=180,
            ...
            return_wpt_in_original = return_entry_wp

            print(f"[AvoidManager] Bypassing '{vehicle.type_id}' via "
                f"{'left' if lane_offset == -1 else 'right'} lane. "
                f"bypass_dist={bypass_distance:.1f}m  return_dist={return_distance:.1f}m")

            self.set_destination(
                bypass_wpt.transform.location,
                waypoint.transform.location
            )
            self.set_destination(
                return_wpt_in_original.transform.location,
                bypass_wpt.transform.location
            )
